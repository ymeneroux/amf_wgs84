...
for i in range(len(POINTS_NAME)):
	...
	LLX.append(max(min(ll_px[0], NY-margin), margin))
	LLY.append(max(min(ll_px[1], NX-margin), margin))
	URX.append(max(min(ur_px[0], NY-margin), margin))
	URY.append(max(min(ur_px[1], NX-margin), margin))
...
out_file_xml = default_out_dir + "/mesures_gcp_correl.xml"
...
OUT_FILES_CSV = []
os.mkdir(default_out_dir+"/csv")
for i in range(len(POINTS_NAME)):
	out_file_csv = default_out_dir+"/csv/"+POINTS_NAME[i]+".csv"
	OUT_FILES_CSV.append(open(out_file_csv, "w"))
	OUT_FILES_CSV[-1].write("image, x, y\n")
...
for f in sorted(os.listdir(input_path_images)):
	...
	#for i in range(6,7):
		...
		OUT_FILES_CSV[i].write(f + ", " + '{:7.3f}'.format(center[0]) + ", " + '{:7.3f}'.format(center[1]) +"\n")
